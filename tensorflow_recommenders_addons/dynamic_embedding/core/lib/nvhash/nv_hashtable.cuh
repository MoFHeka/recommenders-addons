...
template <typename value_type> struct ReplaceOp {
...
__global__ void
insert_kernel(Table *table, const typename Table::key_type *const keys,
...
template <typename Table>
__global__ void
insert_kernel_mask(Table *table, const typename Table::key_type *const keys,
                   const typename Table::mapped_type *const vals,
                   const bool *const status, size_t len) {
...
template <typename Table>
__global__ void
upsert_kernel(Table *table, const typename Table::key_type *const keys,
              const typename Table::mapped_type *const vals, size_t len) {
  const size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < len) {
...
template <typename Table>
__global__ void
accum_kernel(Table *table, const typename Table::key_type *const keys,
             const typename Table::mapped_type *const vals_or_deltas,
             const bool *exists, size_t len) {
...
template <typename Table>
__global__ void
set_kernel(Table *table, const typename Table::key_type *const keys,
           const typename Table::mapped_type *const vals, size_t len) {
...
__global__ void set_kernel_mask(Table *table,
                                ...
                                const typename Table::mapped_type *const vals,
                                const bool *status, size_t len) {

  const size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < len) {
    if (status[i]) {
      auto it = table->find(keys[i]);
      assert(it.Iterator != table->end() &&
             "set_kernel_mask: can't find key, but should exist.");
      ((it.Iterator).getter())->second = vals[i];
    }
  }
}
...
__global__ void
update_kernel(Table *table, const typename Table::key_type *const keys,
...
template <typename Table>
__global__ void
search_kernel(Table *table, const typename Table::key_type *const keys,
              typename Table::mapped_type *const vals, bool *const status,
              size_t len, typename Table::mapped_type *const def_val,
...
__global__ void get_insert_kernel(Table *table,
                                  ...
                                  size_t len, counter_type *d_counter) {
...
__global__ void size_kernel(const Table *table, const size_t hash_capacity,
                            size_t *table_size, KeyType unused_key) {
  ...
  /* Whether the bucket mapping to the current thread is empty? do nothing :
...
__global__ void dump_kernel(KeyType *d_key, ValType *d_val, const Table *table,
                            const size_t offset, const size_t search_length,
                            size_t *d_dump_counter, KeyType unused_key) {

  // inter-block gathered key, value and counter. Global conuter for storing
  ...
  // Each thread gather the key and value from bucket assigned to them and store
  // them into shared mem.
  if (i < search_length) {
    typename Table::value_type val =
        load_pair_vectorized(table->data() + offset + i);
  ...
  // Each block request a unique place in global memory buffer, this is the
  // place where shared memory store back to.
  if (threadIdx.x == 0) {
    global_acc = atomicAdd(d_dump_counter, block_acc);
  }
  __syncthreads();

  // Each thread store one bucket's data back to global memory, d_dump_counter
  // is how many buckets in total dumped.
...
public:
  HashTable(size_t capacity, counter_type count = 0) {
    // assert(capacity <= std::numeric_limits<ValType>::max() && "error: Table
    // is too large for the value type");
    ...
    CUDA_CHECK(cudaMemcpy(d_counter_, &count, sizeof(*d_counter_),
  ...
  void insert(const KeyType *d_keys, const BaseValType *d_vals, size_t len,
              cudaStream_t stream) {
    if (len == 0) {
      return;
    }
  ...
  void get_status(const KeyType *d_keys, bool *d_status, size_t len,
                  cudaStream_t stream) const {
    if (len == 0) {
      return;
    }
    const int grid_size = (len - 1) / BLOCK_SIZE_ + 1;
    get_status_kernel<<<grid_size, BLOCK_SIZE_, 0, stream>>>(table_, d_keys,
                                                             d_status, len);
  }

  void set(const KeyType *d_keys, const BaseValType *d_vals, size_t len,
           cudaStream_t stream) {
    if (len == 0) {
      return;
    }
    const int grid_size = (len - 1) / BLOCK_SIZE_ + 1;
    set_kernel<<<grid_size, BLOCK_SIZE_, 0, stream>>>(
        table_, d_keys, (const ValType *)d_vals, len);
  }
  ...
  void accum(const KeyType *d_keys, const BaseValType *d_vals_or_deltas,
    ...
    const int grid_size = (len - 1) / BLOCK_SIZE_ + 1;
    accum_kernel<<<grid_size, BLOCK_SIZE_, 0, stream>>>(
        table_, d_keys, (const ValType *)d_vals_or_deltas, d_exists, len);
  }
  ...
  void dump(KeyType *d_key, BaseValType *d_val, const size_t offset,
    ...
    assert(block_size > 0 &&
           "nvhash: block_size <= 0, the KV size may be too large!");
    size_t shared_size =
        sizeof(*d_key) * block_size + sizeof(ValType) * block_size;
    const int grid_size = (search_length - 1) / (block_size) + 1;

    dump_kernel<<<grid_size, block_size, shared_size, stream>>>(
        d_key, (ValType *)d_val, table_, offset, search_length, d_dump_counter,
        empty_key);
  }
  ...
  counter_type get_value_head() const {
    ...
    CUDA_CHECK(cudaMemcpy(&counter, d_counter_, sizeof(*d_counter_),
                          cudaMemcpyDeviceToHost));
    return counter;
  }

  void set_value_head(counter_type counter_value) {
    CUDA_CHECK(cudaMemcpy(d_counter_, &counter_value, sizeof(*d_counter_),
                          cudaMemcpyHostToDevice));
  }
  ...
  counter_type add_value_head(counter_type counter_add) {
    ...
    CUDA_CHECK(cudaMemcpy(&counter, d_counter_, sizeof(*d_counter_),
                          cudaMemcpyDeviceToHost));
    counter += counter_add;
    CUDA_CHECK(cudaMemcpy(d_counter_, &counter, sizeof(*d_counter_),
                          cudaMemcpyHostToDevice));
    return counter;
  }

  template <typename GradType, typename Optimizer>
  void update(const KeyType *d_keys, const GradType *d_gradients, size_t len,
              cudaStream_t stream, Optimizer &op) {
    if (len == 0) {
      return;
    }
    const int grid_size = (len - 1) / BLOCK_SIZE_ + 1;
    update_kernel<<<grid_size, BLOCK_SIZE_, 0, stream>>>(table_, d_keys,
                                                         d_gradients, len, op);
  }

  void get_insert(const KeyType *d_keys, void *d_vals, size_t len,
                  cudaStream_t stream) {
    if (len == 0) {
      return;
    }
    const int grid_size = (len - 1) / BLOCK_SIZE_ + 1;
    get_insert_kernel<<<grid_size, BLOCK_SIZE_, 0, stream>>>(
        table_, d_keys, (ValType *)d_vals, len, d_counter_);
  }
  ...
  bool get_lock() {
    ...
    if (update_counter_ > 0) {
      ...
      get_counter_++;
      ret_val =
          true; // There is no update API running, can do get, increase counter
    }
    counter_mtx_.unlock();
    return ret_val;
  }
  ...
  bool update_lock() {
    ...
    if (get_counter_ > 0) {
      ...
      update_counter_++;
      ret_val =
          true; // There is no get API running, can do update, increase counter
    }
    counter_mtx_.unlock();
    return ret_val;
  }
...
private:
  ...
  volatile size_t update_counter_; // How many update APIs are currently called
                                   // on this GPU' hashtable
  volatile size_t get_counter_;    // How many get APIs are currently called on
                                   // this GPU's hashtable
