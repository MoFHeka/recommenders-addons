...
__global__ void gather(const size_t *d_idx, const T1 *d_x_in, const T2 *d_y_in,
                       T1 *d_x_out, T2 *d_y_out, size_t len) {
...
template <typename T>
__global__ void gather(const size_t *d_idx, const T *d_x_in, T *d_x_out,
                       size_t len) {
  const size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < len) {
    const size_t idx = d_idx[i];
...
template <typename T>
__global__ void scatter(const size_t *d_idx, const T *d_x_in, T *d_x_out,
...
template <typename KeyType, typename ValType> struct Resource {
  size_t *d_idx;
  size_t *h_part_sizes;
  size_t *h_offsets;
...
struct Stream_event_resource {
  ...
  // CUDA streams
  cudaStream_t *local_streams; // Local streams for calling GPU to communicate
                               // with other GPUs that contains the hashtable
  cudaStream_t
      *remote_streams; // Remote streams for other GPUs to perform their task
  cudaStream_t
      *caller_stream; // Local stream for calling GPU to perform its own task
};

template <typename KeyType, typename ValType, typename Allocator>
void create_local_resource(Resource<KeyType, ValType> &resource, size_t len,
                           int num_part, Allocator &allocator) {
  ...
  allocator.malloc((void **)&resource.d_keys_parted,
                   len * sizeof(*resource.d_keys_parted));
  allocator.malloc((void **)&resource.d_vals_parted,
                   len * sizeof(*resource.d_vals_parted));
  allocator.malloc((void **)&resource.d_status_parted,
                   len * sizeof(*resource.d_status_parted));
  ...
  /*for (int i = 0; i < num_part; ++i) {
      ...
      CUDA_CHECK(cudaEventCreateWithFlags(&resource.events[i],
...
void create_resource_and_do_partition(const KeyType *d_keys,
                                      ...
                                      size_t len,
                                      const std::vector<int> &gpu_id,
                                      cudaStream_t stream, Allocator &allocator,
                                      KeyGPUMapPolicy_ &policy) {
  const int num_gpu = gpu_id.size();
  create_local_resource(resource, len, num_gpu, allocator);

  size_t *d_part_sizes;
  allocator.malloc((void **)&d_part_sizes, num_gpu * sizeof(*d_part_sizes));
  multisplit(d_keys, resource.d_idx, len, d_part_sizes, num_gpu, stream,
             allocator, policy);
  CUDA_CHECK(cudaStreamSynchronize(stream));

  size_t *h_part_sizes = resource.h_part_sizes;
  size_t *h_offsets = resource.h_offsets;
  CUDA_CHECK(cudaMemcpy(h_part_sizes, d_part_sizes,
  ...
  create_remote_resource(resource, gpu_id, allocator);
}

template <typename T>
void send(const T *d_local, T **d_remote, const size_t *offsets,
          const size_t *part_sizes, cudaStream_t *local_streams,
          const std::vector<int> &gpu_id) {
  ...
  for (int i = 0; i < num_gpu; ++i) {
    ...
    CUDA_CHECK(cudaMemcpyPeerAsync(d_remote[i], gpu_id[i], &d_local[offsets[i]],
                                   dev_local, part_sizes[i] * sizeof(T),
                                   local_streams[i]));
  }
}

template <typename T>
void receive(T *d_local, const T *const *d_remote, const size_t *offsets,
             const size_t *part_sizes, cudaStream_t *remote_streams,
             const std::vector<int> &gpu_id) {
  CudaDeviceRestorer dev_restorer;
  int dev_local = get_dev(d_local);
  assert(dev_local >= 0);
  ...
  for (int i = 0; i < num_gpu; ++i) {
    ...
    CUDA_CHECK(cudaSetDevice(gpu_id[i]));
    CUDA_CHECK(cudaMemcpyPeerAsync(&d_local[offsets[i]], dev_local, d_remote[i],
                                   gpu_id[i], part_sizes[i] * sizeof(T),
                                   remote_streams[i]));
  }
}

void remote_wait_local_streams(cudaEvent_t *events, cudaStream_t *local_streams,
                               cudaStream_t *remote_streams, int num_gpu) {
  for (int i = 0; i < num_gpu; ++i) {
...
void sync_remotes(cudaStream_t *remote_streams,
                  const std::vector<int> &gpu_id) {
  // CudaDeviceRestorer dev_restorer;
...
template <typename KeyType, typename ValType, typename KeyGPUMapPolicy_,
...
public:
  ...
  void set(const KeyType *d_keys, const ValType *d_vals, size_t len,
           Stream_event_resource &s_e_resource) {
    insert_or_set_helper_(d_keys, d_vals, len, &Table_::set, s_e_resource);
  }

  void get(const KeyType *d_keys, ValType *d_vals, bool *d_status, size_t len,
           Stream_event_resource &s_e_resource) const;

  void insert_from_cpu(int gpu_id, const KeyType *h_key, const ValType *h_val,
                       size_t len, const size_t buffer_len,
                       const int buffer_count = 2);

  size_t get_size(int gpu_id) const;

  void dump_to_cpu(int gpu_id, KeyType *h_key, ValType *h_val, size_t len,
                   const size_t buffer_len, const int buffer_count = 2) const;

  void dump_to_gpu(int gpu_id, KeyType *d_key, ValType *d_val,
                   size_t len) const;

  // void accum(const KeyType* d_keys, const ValType* d_vals, size_t len,
  // Stream_event_resource& s_e_resource);
  ...
  void update(const KeyType *d_keys, const GradType *d_gradient, size_t len,
              Optimizer &op, Stream_event_resource &s_e_resource);

  void clear(Stream_event_resource &s_e_resource);

  void remove(const KeyType *d_keys, size_t len,
              Stream_event_resource &s_e_resource);
...
private:
  ...
  using TableFunction_ = void (Table_::*)(const KeyType *d_keys,
                                          const ValType *d_vals, size_t len,
                                          cudaStream_t stream);

  void insert_or_set_helper_(const KeyType *d_keys, const ValType *d_vals,
                             size_t len, TableFunction_ func,
                             Stream_event_resource &s_e_resource);
...
template <typename KeyType, typename ValType, typename KeyGPUMapPolicy_,
          KeyType empty_key>
Stream_event_resource
MultiGpuHashTable<KeyType, ValType, KeyGPUMapPolicy_,
                  empty_key>::stream_event_resource_create(int gpu_id) const {

  /* Check for any invalid input */
  assert(gpu_id >= 0);

  /* We do not check whether the GPU is within the GPU list, user may want to
   * call get/set/insert/accum on non-hashtable GPU */
  ...
  for (int i = 0; i < num_gpu; ++i) {
    CUDA_CHECK(cudaEventCreateWithFlags(&resource.local_events[i],
                                        cudaEventDisableTiming));
  }
  ...
  for (int i = 0; i < num_gpu; ++i) {
    CUDA_CHECK(cudaSetDevice(gpu_id_[i]));
    CUDA_CHECK(cudaEventCreateWithFlags(&resource.remote_events[i],
                                        cudaEventDisableTiming));
  }
...
template <typename KeyType, typename ValType, typename KeyGPUMapPolicy_,
          KeyType empty_key>
void MultiGpuHashTable<KeyType, ValType, KeyGPUMapPolicy_, empty_key>::
    stream_event_resource_destroy(Stream_event_resource &resource) const {
...
template <typename KeyType, typename ValType, typename KeyGPUMapPolicy_,
          KeyType empty_key>
void MultiGpuHashTable<KeyType, ValType, KeyGPUMapPolicy_,
                       empty_key>::insert_from_cpu(int gpu_id,
                                                   const KeyType *h_key,
                                                   const ValType *h_val,
                                                   size_t len,
  ...
  std::vector<int>::iterator iElement =
      std::find(gpu_id_.begin(), gpu_id_.end(), gpu_id);
  ...
  for (int i = 0; i < buffer_count; i++) {
    CUDA_CHECK(cudaMallocManaged((void **)&(d_temp_key[i]),
                                 sizeof(*h_key) * buffer_len));
    CUDA_CHECK(cudaMallocManaged((void **)&(d_temp_val[i]),
                                 sizeof(*h_val) * buffer_len));
  }
  ...
  while (len_counter < len) {
    ...
    int copy_len =
        (len_counter + buffer_len > len ? len - len_counter : buffer_len);

    CUDA_CHECK(cudaMemcpyAsync(d_temp_key[current_stream], h_key + len_counter,
                               sizeof(*h_key) * copy_len,
                               cudaMemcpyHostToDevice,
                               streams[current_stream]));
    CUDA_CHECK(cudaMemcpyAsync(d_temp_val[current_stream], h_val + len_counter,
                               sizeof(*h_val) * copy_len,
                               cudaMemcpyHostToDevice,
                               streams[current_stream]));
    tables_[table_index]->insert(d_temp_key[current_stream],
                                 d_temp_val[current_stream], copy_len,
                                 streams[current_stream]);
...
template <typename KeyType, typename ValType, typename KeyGPUMapPolicy_,
          KeyType empty_key>
size_t
MultiGpuHashTable<KeyType, ValType, KeyGPUMapPolicy_, empty_key>::get_size(
    int gpu_id) const {
  /* Check for any invalid input */
  assert(gpu_id >= 0);
  ...
  /* Calculate which table to use(i.e. The index of "gpu_id" in gpu_id_ vector)
   */
  std::vector<int>::const_iterator iElement =
      std::find(gpu_id_.begin(), gpu_id_.end(), gpu_id);
  assert(iElement != gpu_id_.end());
  int table_index = std::distance(gpu_id_.begin(), iElement);
...
template <typename KeyType, typename ValType, typename KeyGPUMapPolicy_,
          KeyType empty_key>
void MultiGpuHashTable<KeyType, ValType, KeyGPUMapPolicy_,
                       empty_key>::dump_to_cpu(int gpu_id, KeyType *h_key,
                                               ValType *h_val, size_t len,
                                               const size_t buffer_len,
  ...
  for (int i = 0; i < buffer_count; i++) {
    ...
    CUDA_CHECK(cudaMallocManaged((void **)&(d_temp_val[i]),
                                 sizeof(*h_val) * buffer_len));
    CUDA_CHECK(
        cudaMallocManaged((void **)&(d_dump_counter[i]), sizeof(size_t)));
  }

  /* Counters recording how much we have done*/
  const size_t table_capacity =
      tables_[table_index]
          ->get_capacity(); // The Actual capacity of hashtable on gpu_id.
  ...
  while (len_counter < table_capacity) {
    ...
    while (len_counter < table_capacity && pipeline_counter < buffer_count) {
      ...
      int search_length = (len_counter + buffer_len > table_capacity
                               ? table_capacity - len_counter
                               : buffer_len);

      tables_[table_index]->dump(d_temp_key[current_stream],
                                 d_temp_val[current_stream], len_counter,
                                 search_length, d_dump_counter[current_stream],
                                 streams[current_stream]);

      CUDA_CHECK(cudaMemcpyAsync(
          &(h_dump_counter[current_stream]), d_dump_counter[current_stream],
    ...
    while (valid_stream > 0) {
      ...
      CUDA_CHECK(
          cudaMemcpyAsync(h_key + h_ptr, d_temp_key[current_stream],
                          sizeof(*h_key) * h_dump_counter[current_stream],
                          cudaMemcpyDeviceToHost, streams[current_stream]));
      CUDA_CHECK(
          cudaMemcpyAsync(h_val + h_ptr, d_temp_val[current_stream],
                          sizeof(*h_val) * h_dump_counter[current_stream],
                          cudaMemcpyDeviceToHost, streams[current_stream]));
...
template <typename KeyType, typename ValType, typename KeyGPUMapPolicy_,
          KeyType empty_key>
void MultiGpuHashTable<KeyType, ValType, KeyGPUMapPolicy_,
                       empty_key>::dump_to_gpu(int gpu_id, KeyType *d_key,
                                               ValType *d_val,
                                               size_t len) const {
  /* Check for any invalid input*/
  if (len <= 0) {
  ...
  /* Make Sure GPU buffer provided by the user is on the same device as
   * requested GPU or hashtable */
  ...
  tables_[table_index]->dump(d_key, d_val, 0, table_capacity, d_dump_counter,
                             stream);
...
template <typename KeyType, typename ValType, typename KeyGPUMapPolicy_,
          KeyType empty_key>
MultiGpuHashTable<KeyType, ValType, KeyGPUMapPolicy_,
                  empty_key>::MultiGpuHashTable(size_t capacity,
                                                const int *gpu_id,
                                                int gpu_id_len)
    : num_gpu_(gpu_id_len), gpu_id_(gpu_id, gpu_id + gpu_id_len),
      allocator_(gpu_id, gpu_id_len) {
  assert(gpu_id_len > 0);
  ...
  for (int cur = 0; cur < gpu_id_len; ++cur) {
    ...
    for (int peer = 0; peer < gpu_id_len; ++peer) {
      ...
      int can_access;
      CUDA_CHECK(
          cudaDeviceCanAccessPeer(&can_access, gpu_id[cur], gpu_id[peer]));
      if (can_access) {
...
template <typename KeyType, typename ValType, typename KeyGPUMapPolicy_,
          KeyType empty_key>
void MultiGpuHashTable<KeyType, ValType, KeyGPUMapPolicy_, empty_key>::
    insert_or_set_helper_(const KeyType *d_keys, const ValType *d_vals,
                          size_t len, TableFunction_ func,
                          Stream_event_resource &s_e_resource) {
  if (len == 0) {
    return;
  ...
  Resource<KeyType, ValType> resource;
  create_resource_and_do_partition(d_keys, resource, len, gpu_id_,
                                   s_e_resource.caller_stream[0], allocator_,
                                   KeyGPUMapPolicy);

  const int grid_size = (len - 1) / BLOCK_SIZE_ + 1;
  gather<<<grid_size, BLOCK_SIZE_, 0, s_e_resource.caller_stream[0]>>>(
      resource.d_idx, d_keys, d_vals, resource.d_keys_parted,
      resource.d_vals_parted, len);
  CUDA_CHECK(cudaStreamSynchronize(s_e_resource.caller_stream[0]));

  send(resource.d_keys_parted, resource.d_remote_keys, resource.h_offsets,
       resource.h_part_sizes, s_e_resource.local_streams, gpu_id_);
  send(resource.d_vals_parted, resource.d_remote_vals, resource.h_offsets,
       resource.h_part_sizes, s_e_resource.local_streams, gpu_id_);
  remote_wait_local_streams(s_e_resource.local_events,
                            s_e_resource.local_streams,
                            s_e_resource.remote_streams, num_gpu_);

  {
    CudaDeviceRestorer dev_restorer;
    for (int i = 0; i < num_gpu_; ++i) {
      CUDA_CHECK(cudaSetDevice(gpu_id_[i]));
      (tables_[i]->*func)(resource.d_remote_keys[i], resource.d_remote_vals[i],
                          resource.h_part_sizes[i],
...
template <typename KeyType, typename ValType, typename KeyGPUMapPolicy_,
          KeyType empty_key>
void MultiGpuHashTable<KeyType, ValType, KeyGPUMapPolicy_, empty_key>::get(
    const KeyType *d_keys, ValType *d_vals, bool *d_status, size_t len,
    Stream_event_resource &s_e_resource) const {
  if (len == 0) {
    return;
  }
  ...
  const int grid_size = (len - 1) / BLOCK_SIZE_ + 1;
  gather<<<grid_size, BLOCK_SIZE_, 0, s_e_resource.caller_stream[0]>>>(
      resource.d_idx, d_keys, resource.d_keys_parted, len);
  CUDA_CHECK(cudaStreamSynchronize(s_e_resource.caller_stream[0]));
...

  {
    ...
    for (int i = 0; i < num_gpu_; ++i) {
      ...
      CUDA_CHECK(cudaSetDevice(gpu_id_[i]));
      tables_[i]->get(resource.d_remote_keys[i], resource.d_remote_vals[i],
                      resource.d_remote_status[i], resource.h_part_sizes[i],
                      s_e_resource.remote_streams[i]);
    }
  }
  ...
  receive(resource.d_vals_parted, resource.d_remote_vals, resource.h_offsets,
          resource.h_part_sizes, s_e_resource.remote_streams, gpu_id_);

  // Also receive status
  receive(resource.d_status_parted, resource.d_remote_status,
          resource.h_offsets, resource.h_part_sizes,
          s_e_resource.remote_streams, gpu_id_);
  ...
  // cudaEvent_t* remote_events = new cudaEvent_t[num_gpu_];
  {
    ...
    for (int i = 0; i < num_gpu_; ++i) {
      CUDA_CHECK(cudaSetDevice(gpu_id_[i]));
      // CUDA_CHECK(cudaEventCreateWithFlags(&remote_events[i],
      // cudaEventDisableTiming));
      CUDA_CHECK(cudaEventRecord(s_e_resource.remote_events[i],
                                 s_e_resource.remote_streams[i]));
    }
  }

  for (int i = 0; i < num_gpu_; ++i) {
    CUDA_CHECK(cudaStreamWaitEvent(s_e_resource.local_streams[i],
                                   s_e_resource.remote_events[i], 0));
    if (resource.h_part_sizes[i] <=
        0) { // Important! size_t is uint_64, can't be 0 !
      continue;
    }
    ...
    scatter<<<tmp_grid_size, BLOCK_SIZE_, 0, s_e_resource.local_streams[i]>>>(
        resource.d_idx, resource.d_vals_parted, d_vals, resource.h_offsets[i],
        resource.h_part_sizes[i]);

    // Also scatter the status
    scatter<<<tmp_grid_size, BLOCK_SIZE_, 0, s_e_resource.local_streams[i]>>>(
        resource.d_idx, resource.d_status_parted, d_status,
        resource.h_offsets[i], resource.h_part_sizes[i]);
  }
...
/*template<typename KeyType, typename ValType, typename KeyGPUMapPolicy_,
KeyType empty_key> void MultiGpuHashTable<KeyType, ValType, KeyGPUMapPolicy_,
empty_key>::accum(const KeyType* d_keys, const ValType* d_vals, size_t len,
Stream_event_resource& s_e_resource){ if (len == 0) { return;
    }
    ...
    const int grid_size = (len - 1) / BLOCK_SIZE_ + 1;
    gather<<<grid_size, BLOCK_SIZE_, 0,
s_e_resource.caller_stream[0]>>>(resource.d_idx, d_keys, d_vals,
...
template <typename KeyType, typename ValType, typename KeyGPUMapPolicy_,
          KeyType empty_key>
template <typename GradType, typename Optimizer>
void MultiGpuHashTable<KeyType, ValType, KeyGPUMapPolicy_, empty_key>::update(
    const KeyType *d_keys, const GradType *d_gradient, size_t len,
    Optimizer &op, Stream_event_resource &s_e_resource) {
  if (len == 0) {
    return;
  }
  ...
  const int grid_size = (len - 1) / BLOCK_SIZE_ + 1;
  gather<<<grid_size, BLOCK_SIZE_, 0, s_e_resource.caller_stream[0]>>>(
      resource.d_idx, d_keys, d_gradient, resource.d_keys_parted,
      resource.d_vals_parted, len);
  CUDA_CHECK(cudaStreamSynchronize(s_e_resource.caller_stream[0]));
...

  {
    ...
    for (int i = 0; i < num_gpu_; ++i) {
      ...
      CUDA_CHECK(cudaSetDevice(gpu_id_[i]));
      tables_[i]->update(resource.d_remote_keys[i], resource.d_remote_vals[i],
                         resource.h_part_sizes[i],
                         s_e_resource.remote_streams[i], op);
    }
  }
...
template <typename KeyType, typename ValType, typename KeyGPUMapPolicy_,
          KeyType empty_key>
void MultiGpuHashTable<KeyType, ValType, KeyGPUMapPolicy_, empty_key>::clear(
    Stream_event_resource &s_e_resource) {
...
template <typename KeyType, typename ValType, typename KeyGPUMapPolicy_,
          KeyType empty_key>
void MultiGpuHashTable<KeyType, ValType, KeyGPUMapPolicy_, empty_key>::remove(
    const KeyType *d_keys, size_t len, Stream_event_resource &s_e_resource) {
  ...
  // Remove keys on each GPU
  {
    ...
    for (int i = 0; i < num_gpu_; ++i) {
      ...
      CUDA_CHECK(cudaSetDevice(gpu_id_[i]));
      tables_[i]->remove(resource.d_remote_keys[i], resource.h_part_sizes[i],
                         s_e_resource.remote_streams[i]);
    }
  }
