...
__inline__ __device__ long long int
atomicCAS(long long int *address, long long int compare, long long int val) {
...
__inline__ __device__ int64_t atomicAdd(long long *address,
                                        const long long val) {
...
__inline__ __device__ uint64_t atomicAdd(uint64_t *address,
                                         const uint64_t val) {
  return (uint64_t)atomicAdd((unsigned long long int *)address,
                             (unsigned long long int)val);
}

__inline__ __device__ signed char atomicAdd(signed char *address,
...
__device__ __forceinline__ Eigen::half atomicAdd(Eigen::half *address,
...
store_pair_vectorized(pair_type *__restrict__ const ptr, const pair_type val) {
  if (sizeof(uint4) == sizeof(pair_type)) {
    union pair_type2vec_type {
      uint4 vec_val;
      pair_type pair_val;
...
template <typename value_type, typename size_type, typename key_type,
          typename elem_type>
__global__ void init_hashtbl( // Init every entry of the table with <unused_key,
                              // unused_value> pair
    value_type *__restrict__ const hashtbl_values,
    volatile int32_t *valid_marker,
    // lock_type* bucket_lock,
    const size_type n, const key_type key_val, const elem_type elem_val) {
...
template <typename T> struct equal_to {
  ...
  __forceinline__ __host__ __device__ constexpr bool
...
template <typename T> struct iterator_with_index {
  T Iterator;
  size_t current_index;
  __host__ __device__ explicit iterator_with_index(T &input_iterator,
                                                   size_t input_index)
      : Iterator(input_iterator), current_index(input_index) {}
};

template <typename Iterator> class cycle_iterator_adapter {
public:
  ...
  __host__
      __device__ explicit cycle_iterator_adapter(const iterator_type &begin,
  ...
  equal(const cycle_iterator_adapter<iterator_type> &other) const {
 ...
 */
template <typename Key, typename Element, Key unused_key, size_t DIM,
...
public:
  ...
  operator=(const concurrent_unordered_map &) = delete;
  explicit concurrent_unordered_map(size_type n,
                                    const mapped_type unused_element,
                                    const Hasher &hf = hasher(),
                                    const Equality &eql = key_equal(),
                                    const allocator_type &a = allocator_type())
      : m_hf(hf), m_equal(eql), m_allocator(a), m_hashtbl_size(n),
        m_hashtbl_capacity(n), m_collisions(0),
        m_unused_element(unused_element) { // allocate the raw data of hash
                                           // table: m_hashtbl_values,pre-alloc
                                           // it on current GPU if UM.
    m_hashtbl_values = m_allocator.allocate(m_hashtbl_capacity);
    // Allocate marker and lock buffer
    CUDA_RT_CALL(cudaMallocManaged((void **)&valid_marker,
                                   m_hashtbl_capacity * sizeof(*valid_marker)));
    // CUDA_RT_CALL( cudaMalloc((void**)&bucket_lock, m_hashtbl_capacity *
    // sizeof(*bucket_lock)) );
    ...
    init_hashtbl<<<((m_hashtbl_size - 1) / block_size) + 1, block_size>>>(
        m_hashtbl_values, valid_marker, m_hashtbl_size, unused_key,
        m_unused_element);
  ...
  void update_existing_value(volatile mapped_type & existing_value, volatile
  value_type & insert_pair, aggregation_type)
  {
    // update without CAS
  ...
  __forceinline__ __device__ void
  accum_existing_value_atomic(mapped_type &existing_value,
                              const mapped_type &accum_pair) {
  ...
  // specifically for the types where atomicAdd already has an overload.
  // Otherwise the generic update_existing_value will be used. Specialization
  // for COUNT aggregator
  /*
  __forceinline__ __host__ __device__
  void update_existing_value(mapped_type & existing_value, value_type const &
  insert_pair, count_op<int32_t> op)
  {
  ...
  __forceinline__ __host__ __device__
  void update_existing_value(mapped_type & existing_value, value_type const &
  insert_pair, count_op<int64_t> op)
  {
    atomicAdd(&existing_value, static_cast<mapped_type>(1));
  }
  // Specialization for COUNT aggregator
  ...
  __forceinline__ __host__ __device__
  void update_existing_value(mapped_type & existing_value, value_type const &
  insert_pair, count_op<double> op)
  {
    atomicAdd(&existing_value, static_cast<mapped_type>(1));
  }
  */
   ...
   * @Synopsis  Inserts a new (key, value) pair. If the key already exists in
   the map an aggregation operation is performed with the new value and existing
   value. E.g., if the aggregation operation is 'max', then the maximum is
   computed between the new value and existing value and the result is stored in
   the map.
   *
                             ...
                             hash_value_type precomputed_hash_value = 0)
  {
      ...
      // If a precomputed hash value has been passed in, then use it to
  determine
  ...
  insert_key const key_type old_key = atomicCAS( &existing_key, unused_key,
  insert_key);
  ...
  aggregation
        // operation, it is safe to perform the operation when the
  existing_value still
  ...
  functions
        ...
        if ( keys_equal( unused_key, old_key ) || keys_equal(insert_key,
  old_key) ) {
...
); const unsigned long long int unused = converter.longlong; converter.pair = x;
              const unsigned long long int value = converter.longlong;
              const unsigned long long int old_val = atomicCAS(
reinterpret_cast<unsigned long long int*>(tmp_it), unused, value ); if ( old_val
== unused ) { it = tmp_it;
              }
...
x.first ); if ( m_equal( unused_key, old_key ) ) {
  ...
  */
  /*
     ...
     determine
          ...
          while (false == insert_success) {
              ...
              if( unused_key == existing_key ){
                  ...
                  founded = true; // Find is complete: we have know that the
     <k,v> is not inside the hashtable

                  insert_success = true; // Insert is complete: we have found a
     place to hold the <k,v>
              }
              // Situation #2: Current bucket is already the insert key and is a
     valid bucket(not been deleted) else if( valid_marker[current_index] ){ if(
     insert_key == existing_key ){

                      existing_value = x.second; // Update the value to
     latest(i.e "set")

                      founded = true; // Find is complete: we have know that the
     <k,v> is inside the hashtable

                      insert_success = true; // Insert is complete: we have
     found a place to hold the <k,v>
                  }
              }
              else {
                  // Situation #3: Current bucket is already the insert key but
     is a invalid bucket(been deleted) if( insert_key == existing_key ){

                      existing_value = x.second; // Update the value to
     latest(i.e "set")

                      valid_marker[current_index] = true; // Validate the
     current bucket
     ...
     found a place to hold the <k,v>
                  ...
                  // Situation #4: Current bucket is not the insert key and is a
     invalid bucket(been deleted) else{
     ...
     current bucket
                      ...
                      // We have not found the key yet, maybe the key is not
     presented, maybe it is somewhere afterward.

                      insert_success = true; // Insert is complete: we have
     found a place to hold the <k,v>
                  }
              }
              // Situation #5: Current bucket is a valid bucket and is not the
     insert key(already occupied): move to next bucket.
          ...
          // If we have not found the key, we must keep searching to make sure
     there is no duplicated key in the hashtable while( founded == false ){

              // Walked through the rest of the table and couldn't find the key
              if (counter++ >= hashtbl_size) {
                  return iterator(
     m_hashtbl_values,m_hashtbl_values+hashtbl_size, return_hash_bucket);
              }
              ...
              volatile key_type& existing_key = current_hash_bucket->first;
              //volatile mapped_type& existing_value =
     current_hash_bucket->second;

              // If find a unused bucket: we have know that the <k,v> is not
     inside the hashtable if( unused_key == existing_key ){ founded = true;
              }
              else if( insert_key == existing_key ){
                  // If find the valid insert key: we have know that the <k,v>
     is inside the hashtable, invalidate it. if( valid_marker[current_index] ){

                      valid_marker[current_index] = false;

                  }
                  // If find the invalid insert key: we have know that the <k,v>
     is not inside the hashtable. founded = true;
              }
  ...
  insert(const value_type &x, bool precomputed_hash = false,
    ...
    while (false == insert_success) {
      ...
      // Lock the bucket to exclusively access the bucket, we do not use
      // lock-based approach lock_bucket(current_index);

      volatile key_type &existing_key = current_hash_bucket->first;
      mapped_type &existing_value = current_hash_bucket->second;

      // Situation #1: Current bucket is empty key or invalid, we just need to
      // insert. Key is guarantee by user not found in the table.
      ...
      /*if( unused_key == existing_key || valid_marker[current_index] == false
          ...
          insert_success = true; // Insert is complete: we have found a place to
      hold the <k,v>
      }*/

      const key_type old_key =
          atomicCAS((key_type *)&existing_key, unused_key, insert_key);
      ...
      if (unused_key == old_key || x.first == old_key) {
        ...
        // valid_marker[current_index] = true; // Validate the current bucket
        // (actually it is always true if it is unused key)

        insert_success =
            true; // Insert is complete: we have found a place to hold the <k,v>
      } else {
        const int32_t old_valid =
            atomicCAS((int32_t *)(valid_marker + current_index),
                      (int32_t) false, (int32_t) true);
      ...
      // Situation #2: Current bucket is a valid bucket and is not unused
      // key(already occupied): move to next bucket.
  ...
  template <typename hash_value_type = typename Hasher::result_type>
  __forceinline__ __device__ iterator
  accum(const key_type &key, const mapped_type &vals, const bool &exist,
        bool precomputed_hash = false,
        hash_value_type precomputed_hash_value = 0) {
    const size_type hashtbl_size = m_hashtbl_size;
    value_type *hashtbl_values = m_hashtbl_values;
    ...
    while (false == insert_success) {
      ...
      const key_type old_key =
          atomicCAS((key_type *)&existing_key, unused_key, key);
      ...
      if (unused_key == old_key) {
        ...
        if (exist) {
          accum_existing_value_atomic(existing_value,
                                      vals); // Accumlate x to the value
        }
        insert_success =
            true; // Insert is complete: we have found a place to hold the <k,v>
  ...
  __forceinline__ __host__ __device__ iterator_with_index<const_iterator>
  find(const key_type &k) const {
    ...
    iterator_with_index<const_iterator> return_result(
        const_iterator(m_hashtbl_values, m_hashtbl_values + m_hashtbl_size,
                       current_ptr),
        current_index);
  ...
  __forceinline__ __device__ iterator get_insert(
    ...
    while (false == insert_success) {
      ...
      // key, both get and insert fail. Return empty iterator
      if (counter++ >= hashtbl_size) {
        return end();
      }
      ...
      // If old_key == insert_key, this key has already been inserted.
      // In either case, perform the atomic aggregation of existing_value and
      // insert_value Because the hash table is initialized with the identity
      // value of the aggregation operation, it is safe to perform the operation
      // when the existing_value still has its initial value
      // TODO: Use template specialization to make use of native atomic
      // functions
      // TODO: How to handle data types less than 32 bits?
  ...
  /*int assign_async( const concurrent_unordered_map& other, cudaStream_t stream
  = 0 )
  {
  ...
  m_hashtbl_size*sizeof(value_type), cudaMemcpyDefault, stream ) ); return 0;
  }*/

  void clear_async(cudaStream_t stream = 0) {
    constexpr int block_size = 128;
    init_hashtbl<<<((m_hashtbl_size - 1) / block_size) + 1, block_size, 0,
                   stream>>>(m_hashtbl_values, valid_marker, m_hashtbl_size,
  ...
  void print() {
    ...
    CUDA_RT_CALL(cudaHostAlloc((void **)&h_valid_marker,
                               m_hashtbl_capacity * sizeof(*h_valid_marker),
                               cudaHostAllocPortable));
    CUDA_RT_CALL(cudaMemcpy(h_valid_marker, (void *)valid_marker,
                            m_hashtbl_capacity * sizeof(*h_valid_marker),
                            cudaMemcpyDeviceToHost));
    for (size_type i = 0; i < m_hashtbl_size; ++i) {
      std::cout << i << ": " << m_hashtbl_values[i].first << ","
                << m_hashtbl_values[i].second
                << ((h_valid_marker[i]) ? "Valid" : "Invalid") << std::endl;
    }
  ...
  __forceinline__ __host__ __device__ void set_valid(size_type target_index,
                                                     bool target_value) {
  ...
  get_valid(size_type target_index) const {
...
private:
  ...
  // Valid marker buffer
  volatile int32_t *valid_marker;
  // Bucket locks (Currently use uint32_t as lock, may use uint8_t in the
  // future)
  // unsigned int* bucket_lock;

  unsigned long long m_collisions;

  // Bucket lock: atomically lock a bucket, if the bucket is already locked,
  // block until grab the lock(i.e. spining lock)
  /*__forceinline__
  ...
  // Bucket unlock: atomically unlock a bucket, this function must be called by
  // the CUDA thread already own the lock.
  /*__forceinline__
