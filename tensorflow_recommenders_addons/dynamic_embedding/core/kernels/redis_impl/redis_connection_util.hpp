...
template <typename K, typename V> class RedisBaseWrapper {
protected:
...
public:
  ...
  GetKeyBucketsAndOptimizerParamsWithName(const std::string &keys_prefix_name,
                                          const bool only_get_buckets) = 0;

  virtual int CheckSlicesNum(const std::string &keys_prefix_name) = 0;

  virtual std::vector<std::pair<unsigned, unsigned>>
  ClusterNodesSlots(bool full_slots) = 0;

  virtual size_t
  TableSizeInBucket(const std::string &keys_prefix_name_slice) = 0;

  virtual Status
  RemoveHkeysInBuckets(const std::string &keys_prefix_name_slice) = 0;
  ...
  virtual std::unique_ptr<redisReply, ::sw::redis::ReplyDeleter>
  MgetInBucket(const K *, const int64_t begin, const int64_t max_i,
  ...
  virtual Status
  MsetCommand(const K *, const V *values, ThreadContext *thread_context,
              const int64_t begin, const int64_t max_i,
              const int64_t Velems_per_dim0,
              const std::vector<std::string> &keys_prefix_name_slices) = 0;

  virtual Status
  MaccumCommand(const K *, const V *values, const bool *exists,
                ThreadContext *thread_context, const int64_t begin,
                const int64_t max_i, const int64_t Velems_per_dim0,
                std::string &values_dtype_str,
                const std::vector<std::string> &keys_prefix_name_slices) = 0;

  virtual Status
  DelCommand(const K *, ThreadContext *thread_context, const int64_t begin,
             const int64_t max_i,
             const std::vector<std::string> &keys_prefix_name_slices) = 0;
};
...
template <> inline size_t KTypeSize<tstring>(const tstring *in) {
  return in->size();
}
...
template <> inline const char *KContentPointer<tstring>(const tstring *in) {
  return in->data();
}
