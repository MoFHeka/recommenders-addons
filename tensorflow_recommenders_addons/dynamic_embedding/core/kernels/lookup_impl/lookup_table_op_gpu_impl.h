...
#ifndef TFRA_CORE_KERNELS_LOOKUP_TABLE_OP_GPU_IMPL_H_
#define TFRA_CORE_KERNELS_LOOKUP_TABLE_OP_GPU_IMPL_H_

// This header is intended to be included ONLY by the split impl .cu.cc files.
// It pulls in nv_hashtable.cuh and the full TableWrapper template, which are
// the major contributors to long compilation times. By isolating them here,
// each impl file instantiates only 50 DIM values for one K/V type pair and
// all impl files can be compiled in parallel by Bazel.
...
public:
  TableWrapper(size_t max_size) : max_size_(max_size) {
    table_ = std::make_unique<Table>(max_size);
  }
  ...
  size_t rehash_if_needed(const size_t min_capacity, cudaStream_t stream,
    ...
    if (new_capacity != capacity) {
      ...
      table_ = std::make_unique<Table>(new_capacity);
...
#endif // TFRA_CORE_KERNELS_LOOKUP_TABLE_OP_GPU_IMPL_H_
